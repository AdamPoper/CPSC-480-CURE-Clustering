# this is CURE clustering with the dynamic shrinking fraction
...
# the x value is age, the y value is income, and gender will be either red for female and blue for male
class Vertex:
    def __init__(self, _x=0, _y=0, gender=-1):
        self.x = _x
        self.y = _y
        self.age = self.x
        self.income = self.y
...
class Cluster:
    ...
    def calc_density(self):
        ...
        area = x * y
        if area != 0:
            self.density = len(self.vertices) / area        
    def calc_representative_points(self, avg_cluster_density):
        ...
        rep0_distance_vector = reduce_by_scalar_value(rep0_distance_vector, self.density, avg_cluster_density)
        rep1_distance_vector = reduce_by_scalar_value(rep1_distance_vector, self.density, avg_cluster_density)        
        rep_point0 = Vertex()
        rep_point1 = Vertex()        
...
def reduce_by_scalar_value(vec2, cluster_density, avg_density):
    default_shrink_factor = 0.3
    shrink_fraction = 0
    if cluster_density < avg_density:
        shrink_fraction = calc_shrink_below_avg_density(avg_density, cluster_density, default_shrink_factor)
    elif cluster_density > avg_density:
        shrink_fraction = calc_shrink_above_avg_density(avg_density, cluster_density, default_shrink_factor)
    else:
        shrink_fraction = default_shrink_factor    
    vec2['x'] *= shrink_fraction
    vec2['y'] *= shrink_fraction
    return vec2    
...
def draw_cure_graph(clusters):
    ...
    for c in clusters:
        for v in c.vertices:
            if v.gender == 1:
                x_male.append(v.age)
                y_male.append(v.income)
            else:
                x_female.append(v.age)
                y_female.append(v.income)
    ...
    ax.plot(x_male, y_male, 'o', color='blue')  
    ax.plot(x_female, y_female, 'o', color='red')
    plt.show()
...
# if a cluster density is above the average cluster density
def calc_shrink_above_avg_density(avg_density, cluster_density, default_fraction):
    a = default_fraction
    return a - ((-avg_density/cluster_density)+1)*a

# if a cluster is below the average cluster density
def calc_shrink_below_avg_density(avg_density, cluster_density, default_fraction):    
    a = default_fraction
    return ((1-(cluster_density/avg_density))*a)+a
...
# this will return vertices that very obviously have their own clusters
...
# returns vertices all closely grouped in the center
...
def cure_clustering(desired_cluster_count, data_frame):
    clusters = []
    #print(data_frame)
    for i in range(len(data_frame)):
        gender = data_frame.iloc[i]['Gender']
        income = data_frame.iloc[i]['Annual Income (k$)']
        age = data_frame.iloc[i]['Age']
        v = Vertex(age, income, gender)
        cluster = Cluster([v])
        cluster.calc_representative_points(0)        
    ...
    while num_clusters > desired_cluster_count:        
        ...
        for c in clusters:
            c.calc_density()
            avg_cluster_density += c.density
        avg_cluster_density /= len(clusters)
        for c in clusters:
            c.calc_representative_points(avg_cluster_density)            
    draw_cure_graph(clusters)
    
def main():
    desired_cluster_count = 20
    data_frame = pd.read_csv('./data/segmented_customers.csv')
    cure_clustering(desired_cluster_count, data_frame)
